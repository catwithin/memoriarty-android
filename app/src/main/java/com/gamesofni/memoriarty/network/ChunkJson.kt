...
internal fun repeatFromChunk(it: ChunkJson) = Repeat(
    id = it._id,
    dateCreated = stringToDate(it.dateCreated),
    description = it.description,
    toRepeatOn = computeNextRepeat(it),
    project = it.project_id,
//            repeats = it.repeatDates.map { stringToDate(it) }
)

internal fun repeatEntityFromChunk(it: ChunkJson) = RepeatEntity(
    repeatId = it._id,
...
private fun computeNextRepeat(chunk: ChunkJson): Date {
